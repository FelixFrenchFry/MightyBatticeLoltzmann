...
__constant__ FP con_fp_c_x[9];
__constant__ FP con_fp_c_y[9];
__constant__ FP con_w[9];
...
void InitializeConstants()
{
    ...
    cudaMemcpyToSymbol(con_fp_c_x, fp_c_x, 9 * sizeof(FP));
    cudaMemcpyToSymbol(con_fp_c_y, fp_c_y, 9 * sizeof(FP));
    cudaMemcpyToSymbol(con_w, w, 9 * sizeof(FP));
    ...
    const bool write_u_y)
{
    ...
    __shared__ FP tile_df[9][N_BLOCKSIZE];
    ...
    for (uint32_t i = 0; i < 9; i++)
    {
        ...
        u_x += tile_df[i][threadIdx.x] * con_fp_c_x[i];
        u_y += tile_df[i][threadIdx.x] * con_fp_c_y[i];
    }
    ...
    for (uint32_t i = 0; i < 9; i++)
    {
        ...
        FP f_eq_i = con_w[i] * rho * (FP_CONST(1.0)
                  + FP_CONST(3.0) * cu
                  + FP_CONST(4.5) * cu * cu
                  - FP_CONST(1.5) * u_sq);
        ...
        uint32_t dst_idx = (src_y + con_c_y[i]) * N_X
                         + ((src_x + con_c_x[i] + N_X) % N_X);

        // stream df value to the destination in dir i
        dvc_df_new[i][dst_idx] = f_new_i;
    }
}
    ...
    const bool write_u_y)
{
    ...
    if (is_SWD)
    {
        FFLU_ShearWaveDecay_Push_Inner_K<N_BLOCKSIZE><<<N_GRIDSIZE, N_BLOCKSIZE>>>(
    ...
    if (!kernelAttributesDisplayed_inner)
    {
        if (is_SWD)
        {
            DisplayKernelAttributes(FFLU_ShearWaveDecay_Push_Inner_K<N_BLOCKSIZE>,
...
// =============================================================================
// fully fused lattice update kernel for SWD sim (outer cells only)
// (for applying the pbc and populating the hallo cells)
// =============================================================================
template <uint32_t N_BLOCKSIZE>
__global__ void FFLU_ShearWaveDecay_Push_Outer_K(
    ...
    FP* const* __restrict__ dvc_df_halo_bot,
    FP* __restrict__ dvc_rho,
    FP* __restrict__ dvc_u_x,
    FP* __restrict__ dvc_u_y,
    const FP omega,
    ...
    const bool write_u_y)
{
    ...
    for (uint32_t i = 0; i < 9; i++)
    {
        ...
        // (with respect to pbc and halo cells)
        int dst_x = (src_x + con_c_x[i] + N_X) % N_X;
        ...
        {
            dvc_df_new[i][dst_y_raw * N_X + dst_x] = f_new_i;
        }
    }
}
    ...
    const bool write_u_y)
{
    ...
    for (uint32_t i = 0; i < N_DIR; i++)
    {
            ...
            (con_c_y[i] ==  1 && src_y_global == N_Y_TOTAL - 1))   // into top wall
        {
            ...
            {
                // TODO: correct equation w.r.t. omega and con_w[i] ?
                f_new_i -= FP_CONST(6.0) * con_w[i] * rho * con_fp_c_x[i] * u_lid;
            }
    ...
    const bool write_u_y)
{
    ...
    if (is_SWD)
    {
        FFLU_ShearWaveDecay_Push_Outer_K<N_BLOCKSIZE><<<N_GRIDSIZE, N_BLOCKSIZE>>>(
            dvc_df, dvc_df_new, dvc_df_halo_top, dvc_df_halo_bot, dvc_rho,
            dvc_u_x, dvc_u_y, omega, N_X, N_Y, N_CELLS_OUTER,
    ...
    if (!kernelAttributesDisplayed_outer)
    {
        if (is_SWD)
        {
            DisplayKernelAttributes(FFLU_ShearWaveDecay_Push_Outer_K<N_BLOCKSIZE>,
