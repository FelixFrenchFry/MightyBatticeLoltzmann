...
__constant__ int dvc_dvk_c_x[9];
__constant__ int dvc_dvk_c_y[9];
bool constantsInitialized_DVK = false;

void InitializeConstants_DVK()
{
    // one-time initialization guard
    if (constantsInitialized_DVK) { return; }
    ...
    cudaMemcpyToSymbol(dvc_dvk_c_x, c_x, 9 * sizeof(int));
    cudaMemcpyToSymbol(dvc_dvk_c_y, c_y, 9 * sizeof(int));

    cudaDeviceSynchronize();
    constantsInitialized_DVK = true;
}
...
__global__ void ComputeDensityAndVelocityField_K(
    ...
    const uint32_t N_CELLS)
{
    ...
    for (uint32_t i = 0; i < N_DIR; i++)
    {
        ...
        sum_x += df_i * dvc_dvk_c_x[i];
        sum_y += df_i * dvc_dvk_c_y[i];
    }
...
void Launch_DensityAndVelocityFieldComputation(
    ...
    const uint32_t N_CELLS)
{
    InitializeConstants_DVK();

    const uint32_t N_GRIDSIZE = (N_CELLS + N_BLOCKSIZE - 1) / N_BLOCKSIZE;

    ComputeDensityAndVelocityField_K<N_DIR, N_BLOCKSIZE><<<N_GRIDSIZE, N_BLOCKSIZE>>>(
