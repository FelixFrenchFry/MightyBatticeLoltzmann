...
__global__ void ComputeFullyFusedOperations_K(
    const float* __restrict__ dvc_df,
    float* __restrict__ dvc_df_next,
    ...
    const uint32_t N_CELLS)
{
    ...
    for (uint32_t i = 0; i < N_DIR; i++)
    {
        ...
        float f_new_i = dvc_df[base_idx + i] * (1 - omega) + omega * f_eq_i;
        ...
        dvc_df_next[dst_idx * 9 + i] = f_new_i;
    }
}
